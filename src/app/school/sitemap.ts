...
// Refresh each shard hourly — keeps newly added schools in the index
// without waiting for a full redeploy.
export const revalidate = 3600;

async function getTotalSearchPagesCount() {
  try {
    const res = await getSiteMapData({ isSchoolOnly: true });
  ...
  try {
    ...
    if (slugs.data)
      return slugs.data
        ...
        .map((slug) => ({
          url: `${SITE_BASE_URL}/school/${slug.slug}`,
    ...
    console.log(`error in school sitemap shard ${id}=>`, error);
    return [];
  }
}
